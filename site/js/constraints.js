...
function drawLine(svg, cells, color, width) {
    ...
    polyline.setAttribute('stroke', `rgba(${color[0]}, ${color[1]}, ${color[2]}, ${color[3] / 255})`);
...
function drawKillerCage(svg, cells, sum, color) {
    ...
    cellCoords.forEach(({ row, col }) => {
        ...
        rect.setAttribute('fill', `rgba(${color[0]}, ${color[1]}, ${color[2]}, 0.1)`);
        rect.setAttribute('stroke', `rgba(${color[0]}, ${color[1]}, ${color[2]}, 0.8)`);
...
function drawCircleBetweenCells(svg, cell1Pos, cell2Pos, color) {
    ...
    circle.setAttribute('fill', `rgba(${color[0]}, ${color[1]}, ${color[2]}, ${color[3] / 255})`);
...
function renderConstraint(svg, constraint) {
    ...
    switch (type) {
        ...
        case 'kropki':
            const color = constraint.color === 'black' ? [0, 0, 0, 255] : [255, 255, 255, 255];
